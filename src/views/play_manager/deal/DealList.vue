<template>
  ...
  >
    <SourcingPlayCard
      :sourcing="sourcing.SourcingPlayInfo"
        ...
        <div class="card-header">
          <span>
            {{
              "发布时间" +
              ": " +
              formatTime(sourcing.SourcingPlayInfo.CreateTime)
            }}
          </span>
          <el-button link>
            <el-icon
...
import { getPlayDealList } from "../../../api/deal";
...
const getPendingSourcingList = () => {
  console.log(page);
  getPlayDealList(page).then((res: any) => {
    if (res) {
