    ...
    >
      ...
      <template #managerRes>
        ...
        >
          ...
          <el-divider style="margin-top: 1em; margin-bottom: 1em"></el-divider>
          <PlayDetails
...
const initRequest = (name: string) => {
  switch (name) {
    case "NotServicePlayOrders":
      pageRequest.OrderStatus = 1;
      break;
    case "InServicePlayOrders":
      pageRequest.OrderStatus = 5;
      break;
    case "CompletedPlayOrders":
...
const getOrders = () => {
  getOrderList(pageRequest).then((res: any) => {
    if (res.code == 1) {
      ...
      orderList.value.forEach((element: any) => {
        if (element.RefundInfo && element.RefundInfo.Pictures.length) {
          element.RefundInfo.Pictures = element.RefundInfo?.Pictures.map(
            (url: any) => {
              //给url加上域名
        ...
        element.OrderObject.PlayDetails = JSON.parse(
          element.OrderObject.PlayDetails
        );
        element.OrderObject.PlayAmenitys =
          element.OrderObject.PlayAmenitys.split(",");
        element.OrderObject.PlayItems =
          element.OrderObject.PlayItems.split(",");
        element.OrderObject.PicList = element.OrderObject.PicList.map(
          (pic: any) => {
            return pic.Url;
          }
        );
    ...
    .then(() => {
      checkIn({ orderId: orderId }).then((res: any) => {
        ...
        if (res.code === 1) {
          ...
          setTimeout(() => {
            router.push({ name: "InServicePlayOrders" });
          }, 300);
        } else {
