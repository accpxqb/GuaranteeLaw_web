  ...
  <div>
    ...
    >
      <!-- <h3 class="loginTitle" v-if="loginForm.userType == '0'">
                {{ $t('loginVue.emailRegister')}}
            </h3>
            <h3 class="loginTitle" v-else>
                {{ $t('loginVue.emailSettle')}}
            </h3> -->
      ...
      <el-form-item prop="code">
        ...
        >
          <template #append>
            <el-button type="primary" v-if="!isSend" @click="submitSendCode">{{
              $t("button.mailboxVerification")
            }}</el-button>
            <el-button type="primary" v-else @click="submitSendCode"
...
<script>
import { mailboxVerification, register } from "@/api/login";
export default {
  props: {
    userType: String,
  },
    ...
    return {
      ...
      rules: {
        ...
        code: [
          {
            required: true,
            message: this.$t("loginVue.enterCode"),
            trigger: "blur",
          },
