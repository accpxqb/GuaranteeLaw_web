<template>
    <el-card shadow="never">
        <el-form :model="form" ref="formRef" :rules="rules" label-position="top">
            <el-form-item>
                <el-input v-model="form.CommentContent" :rows="4" type="textarea" placeholder="分享体验心得，给其他人一个参考~"
                    resize="none" />
            </el-form-item>
            <el-form-item label="上传图片">
                <el-upload v-model:file-list="fileList" list-type="picture-card" :action="uploadUrl" :headers="headers"
                    :accept="accept" :on-preview="handlePictureCardPreview" :on-remove="handleRemove"
                    :before-upload="beforePicUpload" :on-success="handlePicSuccess" :limit="5" class="uploadimglist">
                    <el-row>
...
import { type UploadProps, type UploadUserFile, type FormInstance, type FormRules, ElMessage } from 'element-plus'
import { beforePicUpload } from '@/utils/uploadHelper'
import { deletePicture, deletePictures, followUp } from '@/api/comment'
import config from '@/utils/config'
import { useRoute, useRouter } from 'vue-router'
import type { CommentInfo, CommentModel } from '@/types/comment'
onUnmounted(() => {
    console.log('unmounted')
    if (form.PicUrlList.length > 0)
        deletePictures(form.PicUrlList)
})
...
const handleRemove: UploadProps['onRemove'] = (uploadFile: any, uploadFiles) => {
    ...
    deletePicture({ url }).then((res: any) => {
        if (res.code === 1) {
            ...
            if (index !== -1)
                form.PicUrlList.splice(index, 1)
