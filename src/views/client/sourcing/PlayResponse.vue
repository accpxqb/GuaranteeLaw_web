<template>
  ...
  >
    ...
    <template v-for="(play, index) in item.PlayPropertyList" :key="play.Id">
      <el-divider v-if="index > 0" style="width: 110%; margin-left: -20px">
      </el-divider>
      <PlayInfo :play="play"></PlayInfo>
      <PlayDetails :play-details="play.PlayDetails"></PlayDetails>
      ...
      >
        <span
          :style="{
            fontSize: 'small',
            color: timeColor([play.BeginTime, play.EndTime]),
          }"
        >
          {{
            `开始时间：
             ${formatDate(play.BeginTime)}
              -  
             结束时间：
             ${formatDate(play.EndTime)}`
          }}
        </span>
        <div>
          <span>共支付：</span>
          ...
          <el-button
            @click="handleShowEditDialog(play)"
            v-if="play.IsModification == 0"
          >
            我要改单
          </el-button>
          <el-button v-if="play.IsModification == 1" plain disabled>
            回复中…
          </el-button>
          <el-button
            @click="handleShowEditDialog(play)"
            v-if="play.IsModification == 2"
          >
            继续改单
          </el-button>
          <el-button color="#ff5800" @click="btnDeal(play)"> 下单 </el-button>
        </div>
      </div>
    </template>
  </el-card>
...
import { getSourcingPropertyList } from "@/api/sourcing_play.js";
...
import { modifyProperty } from "@/api/sourcing_play.js";
import { ElMessage, ElMessageBox } from "element-plus";
import { createPlayOrder } from "@/api/orderplay.js";
...
const btnRouter = (managerId:any) => {
  ...
  var path = router.resolve({
    ...
    query: { managerId: managerId ,managerType:ManagerType.乐玩},
  });
  ...
  getSourcingPropertyList({ sourcingId: props.sourcingId }).then((res: any) => {
    if (res.code == 1) {
      ...
      responseList.value.forEach((item: any) => {
        item.PlayPropertyList.forEach((element: any) => {
          element.PicList = element.Urls;
          element.PlayItems = element.PlayItems.split(",");
          element.PlayAmenitys = element.PlayAmenitys.split(",");
          element.PlayDetails = JSON.parse(element.PlayDetails);
        });
      });
    }
  });
